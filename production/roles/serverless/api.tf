...
resource "aws_api_gateway_stage" "public_api_gateway_stage" {
  deployment_id = aws_api_gateway_deployment.public_api_gateway_deployment.id
  rest_api_id   = aws_api_gateway_rest_api.public_api.id
...
resource "aws_cloudwatch_log_group" "public_api_log_group" {
  name              = "${module.label.id}-API-Gateway-Access-Logs_${aws_api_gateway_rest_api.public_api.id}/${terraform.workspace}"
