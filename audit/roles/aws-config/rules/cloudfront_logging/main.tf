##
# ORG Custom Config Rule to check whether your Amazon CloudFront Distributions
...
module "lambda_label" {
  ...
  attributes  = ["cloudfront", "logging", data.aws_region.current.name]
}
...
data "archive_file" "lambda_package" {
  type        = "zip"
  source_file = "${path.module}/files/cloudfront_logging/cloudfront_logging.py"
  output_path = "${path.module}/files/cloudfront_logging.zip"
}
...
resource "aws_lambda_function" "default" {
  ...
  handler          = "cloudfront_logging.lambda_handler"
  memory_size      = 128
  source_code_hash = data.archive_file.lambda_package.output_base64sha256
  runtime          = "python3.8"
  timeout          = 60
  description      = "Checks whether your Amazon CloudFront Distributions has proper logging configuration. The rule returns NON_COMPLIANT if the 'Logging' is not set to 'True' and an S3 bucket is not specified as a target."
...
resource "aws_config_organization_custom_rule" "cloudfront_logging" {
  depends_on = [
