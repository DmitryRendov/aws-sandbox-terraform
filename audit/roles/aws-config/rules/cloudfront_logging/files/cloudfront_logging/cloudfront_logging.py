"""Checks whether your Amazon CloudFront Distributions has proper logging configuration. The rule returns NON_COMPLIANT if the 'Logging' is not set to 'True' and an S3 bucket is not specified as a target.
"""
...
def get_distribution_config(cf, id):
    return cf.get_distribution_config(Id=id)
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    for distribution in distributions:
        config = get_distribution_config(cf, distribution["Id"])
        if 'DistributionConfig' in config and 'Logging' in config['DistributionConfig']:
            if config['DistributionConfig']['Logging']['Enabled']:
                evaluations.append(build_evaluation(distribution['Id'], 'COMPLIANT', event, annotation='''CloudFront Distribution "{}" has proper Logging configuration.'''.format(distribution["Id"])))
            else:
                evaluations.append(build_evaluation(distribution['Id'], 'NON_COMPLIANT', event, annotation='''CloudFront Distribution "{}" has Logging option disabled. Please configure!'''.format(distribution["Id"])))
