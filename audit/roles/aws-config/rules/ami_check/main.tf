##
# ORG Custom Config Rule to check for AMI compliance.
#
...
module "lambda_label" {
  ...
  attributes  = ["ami", "check", data.aws_region.current.name]
}
...
data "archive_file" "lambda_package" {
  type        = "zip"
  source_file = "${path.module}/files/ami_check/ami_check.py"
  output_path = "${path.module}/files/ami_check.zip"
}
...
resource "aws_lambda_function" "default" {
  ...
  handler          = "ami_check.lambda_handler"
  memory_size      = 128
  source_code_hash = data.archive_file.lambda_package.output_base64sha256
  runtime          = "python3.8"
  timeout          = 60
  description      = "Lambda for Custom Config Rule to check for AMI compliance."
...
resource "aws_config_organization_custom_rule" "ami_check" {
  depends_on = [
  ...
  description         = "Custom Config Rule to check for AMI compliance."
