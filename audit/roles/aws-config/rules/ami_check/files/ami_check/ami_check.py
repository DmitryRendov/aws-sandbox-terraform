"""
This Lambda was forked from:
https://github.com/awslabs/aws-config-rules/blob/master/python/AMI_OUTDATED_CHECK/AMI_OUTDATED_CHECK.py

The original script only checks age of AMIs, this version will instead
take a list of ami prefixes and calculate based on environment and
backoff period what the whitelisted amis should be.

Rule Name:
    AMI_OUTDATED_CHECK

Description:
    Check all instances with private AMIs are running the AMI.
...
Rule Parameters:
    ...
    | -------------------- | --------- | --------------------------------------------- | ------------------|
    | AmiAccountId         | Required  | Account number that owns the Amis.            | must be number.   |
    | -------------------- | --------- | --------------------------------------------- | ------------------|
    | AmiBakeTimeDays      | Required  | Time in days that Amis must bake before use.  | must be number.   |
    | -------------------- | --------- | --------------------------------------------- | ------------------|
    | AmiPrefixes          | Required  | Comma separated list of Ami prefixes that we  | must be equal to  |
    |                      |           | Will scan for.                                | item in list.     |
    | -------------------- | --------- | --------------------------------------------- | ------------------|
...
logger = logging.getLogger("ami_check")
...
# Main Code #
#############
def get_approved_amis(event, valid_rule_parameters):
    """Compile and return a list of approved amis based on AmiPrefixes"""
    ec2_client = get_client('ec2', event)

    approved_amis = []
    ami_prefixes = valid_rule_parameters['AmiPrefixes'].split(',')
    for ami_prefix in ami_prefixes:
        logger.info(f"Determining approved ami for prefix {ami_prefix}")
        images = ec2_client.describe_images(
            Owners=[str(valid_rule_parameters['AmiAccountId'])],
            Filters=[
                {
                    'Name': 'name',
                    'Values': [f"{ami_prefix}*"]
        ...
        images.sort(reverse=True, key=lambda i: i['CreationDate'])

        for image in images:
            # Go through images and add the first one that
            # has baked long enough to our list of approved amis
            creation_date = parser.parse(image['CreationDate'])
            current_date = datetime.now(creation_date.tzinfo)
            bake_time = timedelta(days=valid_rule_parameters['AmiBakeTimeDays'])
            baked_date = creation_date + bake_time

            if baked_date <= current_date:
                # This image has baked
                logger.info(f"Adding image id {image['ImageId']} to approved AMIs")
                approved_amis.append(image['ImageId'])
                break
    return approved_amis
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    """
    approved_amis = get_approved_amis(event, valid_rule_parameters)
    ec2_client = get_client('ec2', event)
    evaluations = []
    ...
    if configuration_item:
        ...
        logger.debug(image_lookup)

        ami_prefixes = valid_rule_parameters['AmiPrefixes'].split(',')
        # Now loop through the instances again and determine the compliance status,
        # appending it to our evaluations list.
        for instance in instance_array:
            if not instance['ImageId'] in image_lookup:
                logger.info(f"{instance['InstanceId']} has an AMI we cannot look up, Skipping.")
                continue
            elif not instance['State']['Name'] == "running":
                logger.info(f"Instance {instance['InstanceId']} is not runnning, Skipping.")
                continue
            elif not image_lookup[instance['ImageId']]['Name'].startswith(tuple(ami_prefixes)):
                ami_name = image_lookup[instance['ImageId']]['Name']
                logger.info(f"{instance['InstanceId']} has an AMI we are not tracking with name "
                      f"{ami_name}, Skipping.")
                continue
            elif instance['ImageId'] in image_lookup:
                status, annotation = evaluate_image(
...
def evaluate_image(ami, approved_amis, instance_id, valid_rule_parameters):
    image_whitelist = valid_rule_parameters['WhitelistedAmis'].split(",")

    #Scenario 9 - Whitelisted AMI
    if ami['ImageId'] in image_whitelist:
        logger.info(f"Instance {instance_id} with AMI {ami['ImageId']} matched an image in the"
              f" Image Whitelist.")
        return 'COMPLIANT', "ImageId in AMI Whitelist"

    logger.info(f"Comparing Instance {instance_id} with AMI {ami['ImageId']} with images in the"
          f" Approved AMI List.")
    if ami['ImageId'] in approved_amis:
        logger.info(f"Instance {instance_id} is running an approved AMI.")
        return 'COMPLIANT', "ImageId in Approved AMI list"
    logger.info(f"Instance {instance_id} is not running an approved AMI.")
    return 'NON_COMPLIANT', "ImageId is not in Approved AMI List"
...
def evaluate_parameters(rule_parameters):
    ...
    """
    # Validate AmiBakeTimeDays parameter.
    if 'AmiBakeTimeDays' not in rule_parameters:
        raise ValueError('The Config Rule must have the parameter "AmiBakeTimeDays"')

    if not rule_parameters['AmiBakeTimeDays']:
        rule_parameters['AmiBakeTimeDays'] = 0
    ...
    try:
        rule_parameters['AmiBakeTimeDays'] = int(rule_parameters['AmiBakeTimeDays'])
    except ValueError:
        raise ValueError('The parameter "AmiBakeTimeDays" must be a integer')

    if rule_parameters['AmiBakeTimeDays'] < 0:
        raise ValueError('The parameter "AmiBakeTimeDays" must be greater than or equal to 0')

    # Validate AmiAccountId parameter.
    if 'AmiAccountId' not in rule_parameters:
        raise ValueError('The Config Rule must have the parameter "AmiAccountId"')

    #The int() function will raise an error if the string configured can't be converted to an integer
    #no rounding, truncating, or modification.
    try:
        rule_parameters['AmiAccountId'] = int(rule_parameters['AmiAccountId'])
    except ValueError:
        raise ValueError('The parameter "AmiAccountId" must be a integer')

    if len(str(rule_parameters['AmiAccountId'])) != 12:
        raise ValueError('The parameter "AmiAccountId" must be a 12-digit number')

    #Validate AmiPrefixes parameter
    if 'AmiPrefixes' not in rule_parameters:
        raise ValueError(
            'The Config Rule must have the parameter "AmiPrefixes"'
        )
    else:
        if not isinstance(rule_parameters['AmiPrefixes'], str):
            raise ValueError(
                'AmiPrefixes must be a string or a list of strings separated by comma.'
            )
        ...
        for evaluation in compliance_result:
            ...
            for field in ('ComplianceResourceType', 'ComplianceResourceId', 'ComplianceType', 'OrderingTimestamp'):
                if field not in evaluation:
                    logger.info(f"Missing {field} from custom evaluation.")
                    missing_fields = True
